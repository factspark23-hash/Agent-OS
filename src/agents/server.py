...
class AgentServer:
    ...
    async def _handle_commands_list(self, request: web.Request) -> web.Response:
        ...
        commands = {
            ...
            "get-dom": {
                "params": {},
                "description": "Get structured DOM snapshot"
            },
            "scroll": {
            ...
            "transcribe": {
                "params": {"url": "string", "language": "string (optional)"},
                "description": "Transcribe video/audio from URL"
            },
            "auto-login": {
                "params": {"url": "string", "domain": "string"},
                "description": "Auto-login using stored credentials"
            },
            "save-creds": {
                "params": {"domain": "string", "username": "string", "password": "string"},
                "description": "Save credentials for auto-login"
            },
            "tabs": {
                "params": {"action": "list|new|close", "tab_id": "string (optional)"},
    ...
    async def _handle_debug(self, request: web.Request) -> web.Response:
        """Handle HTTP GET /debug — show real-time logs and stats."""
        return web.json_response({
    ...
    async def _cmd_tabs(self, data: Dict, session) -> Dict:
        ...
        if action == "list":
            ...
            tid = await self.browser.new_tab(tab_id or f"tab-{len(self.browser._pages)}")
